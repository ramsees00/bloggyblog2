...
:END:
I recently released [deft](https://github.com/sstraust/deft), a new library for managing type and protocol defs, and so, to take it for a spin, I decided to write the game of chess in Clojure.
...
I hand-drew all of the pieces, and used this little reagent snippet
   ...
   (doall (for [x (range (count tiles))]
		  ...
		  [:v-box
		   (doall (for [y (reverse (range (count (nth tiles x))))]
			...
			"")]))]))]
#+end_src
to render the display, but kind of the interesting bit is how to represent the board state and all of its pieces.
...
is-valid-move-for-piece?, which returns a true-false value of "can my piece move to this square", works great for most pieces, but is insufficient for more complicated moves, like castling, where several pieces move at the same time. To handle this, we use another multimethod
...
I'm not going to write it all out here in the post, but it was pretty straightforward to define relevant methods for all the different pieces types and special-cases, and I'm pretty happy with how the code turned out. When you're reading this code, it really looks like somebody just wrote down all the rules for the game of chess, and that's what makes me feel good, and feel like we've done things in a way that's well organized.
...
- because deft gives us features from both the protocols/records ecosystem and the multimethods/plain-maps ecosystem, we didn't need to *decide* which tool was right for our use-case. this removes just like a lot of cognitive load, because we can use one tool, and trust that it can do everything pretty well.
