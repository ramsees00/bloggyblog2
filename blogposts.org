#+TITLE: Sammy's Blog
#+RSS_LINK_HOME: https://www.sammystraus.com/
#+HTML_HEAD: <style> body {width:80%; margin: auto; max-width:38rem} blockquote { background-color: #f2f2f2; border: 1px solid #e6e6e6; border-radius: 3px; padding: 8pt 12pt; } [class^="section-number-"]{display:none}</style>
#+OPTIONS:  toc:1
#+HTML: <div class="rss-link"><a href="feed.xml">rss</a></div>
# use org-export-blog to export the blog to html and rss
# and then git push origin master to publish it
* Using LLMs to Make the Emacs Web Browser Great Again
:PROPERTIES:
:CUSTOM_ID: using-llms-to-make-the-emacs-web-browser-great-again
:RSS_PERMALINK: #using-llms-to-make-the-emacs-web-browser-great-again
:PUBDATE:  2026-08-24
:ID:       3F5C9554-EEBA-4F5D-BD44-1623161F6CD0
:END:
When I tell people about the emacs web browser, their first reaction is 'Wait emacs has a web browser?!', followed by, 'yeah... I guess that's kind of a web browser'.

The emacs web browser is designed to render webpages in a terminal-based, textonly environment and, well, it goes about exactly the way you'd expect. It works well on HTML-only, small-content websites, but not so good for anything else.

The problem is that emacs is primarily a text-only editor, but the modern internet is very much /not/ plain-text based. /Simple/ HTML, designed in the /simple/ style of the 1990s has a clear translation into plain text, but /modern/ UI design, with it's visual styling, heavy javascript, and just, overall chaos, not so much.


The emacs web browser would be great, if only we could find a way to go back in time...


Ok, so the plan is simple. We take modern webpages, rewrite their source code into old-school simply-styled HTML, and then pipe it through to the emacs web browser. We just need to find a way to do this easily and at scale.

So um, you didn't forget about LLMs did you? It's nice to be using them in a way that will hopefully /reduce/ the amount of spam content in the world.

Basically, the plan is we write a program that:
- Opens a webpage in firefox
- Loads the page and runs all the javascript
- Asks an LLM to generate a *re-usable program* that transforms the current-page's contents into simplified text
- Inject the generated javascript into the firefox browser
- Grab the new page source and forward it to emacs web browser.

Yeah, and so, I've been using it, and it runs very well. Like realistically I can use this on many webpages where eww (emacs web wowser) did not run previously.

Below are some examples of before and after:

The docs for Clojure's mapcat
#+CAPTION: (before) Using eww-readable
#+ATTR_HTML: :width 400px
[[file:./images/ClojureDocs_before.png]]

#+CAPTION: (after) Using simpleweb-simplify-page
#+ATTR_HTML: :width 400px
[[file:./images/ClojureDocs_after.png]]


It takes a moment to generate the initial modifier program, but once it's generated, it runs pretty fast, and during day-to-day browsing, it mostly doesn't need any extra LLM calls. I have been using it myself and enjoying the experience.


The source code lives here: https://github.com/sstraust/simpleweb

and is open-source, and free to install, etc., etc.
...
:PROPERTIES:
:CUSTOM_ID: i-m-unsure-if-someone-yoinked-my-project-idea
:RSS_PERMALINK: #i-m-unsure-if-someone-yoinked-my-project-idea
:ID:       88DAAFA1-34C7-458F-86D3-901B079021BD
:PUBDATE:  2026-08-23
:END:
I applied to give a talk this year at Clojure Conj. After good bit of work, I submitted the following blurb:

#+begin_quote
A Practical Type Linter for Clojure

Type validation is great, but manually writing annotations is painful. In this talk we explore the idea of a gradual type /linter/ that uses strong inference rules, along with relaxed soundness requirements, to provide useful type warnings without adding annotations or changes to existing code.

We also give an overview of the existing ecosystem of tools, from schema to spec to malli to typedclojure. Complete with live demos!  
#+end_quote

I also gave speaker notes, explaining that I was adapting ideas from Hindley Milner + constraint based typing to serve as the linter. My talk was rejected from the conference on July 1, and then, 15 days later, the maintainer of clj-kondo posted this to the Clojurian's slack:

#+ATTR_HTML: :width 600px
[[./clj-kondo-slack.png]]

And so now, I'm unsure if somebody yoinked my idea.

I generally like to believe in the well-intentionedness of people, especially everyone in the Clojure community, so it's possible that 1 of 3 things happened:
1. This is completely coincidental. Nobody saw my talk submission, and I just happened to get extremely unlucky.
2. Someone who is not working on clj-kondo saw my submission, absorbed the ideas, and talked about them in passing. No bad intentions were made by anybody involved, but still the information somehow flowed through indirectly.
3. Somebody yoinked my talk submission, or saw my talk and was heavily inspired.


The good news in all of this is that, by all accounts, borkdude is an overwhelmingly good dude. I don't think he would have yoinked something without attribution, and he is in general welcoming and active at linking to the inspiration for his work.

So that kind of leaves scenario 1 and scenario 2. Based on the timing of it, I really think scenario 2 could have happened through no fault of anyone's, but it's hard to tell, and I don't think anyone can ever really trace sort of the indirect flow of information.


What's frustrating though, is that, I don't know, and I don't think I'll ever really figure it out.


I think maybe I just feel frustrated that my project got over-shadowed a month or so before I planned to announce it, but, you know, maybe sometimes you just get really unlucky.


* If technology got better, why did everything get worse?
:PROPERTIES:
:CUSTOM_ID: if-technology-got-better-why-did-everything-get-worse
:RSS_PERMALINK: #if-technology-got-better-why-did-everything-get-worse
:PUBDATE:  2026-07-30
:ID:       91AF6B8C-A508-4432-A6A8-BAED1B93D8BC
:END:
...
1. touch grass. really when the world is like this, i think the best solution is to just leave. it's not easy, especially when the planning is often online, but if you can set it up, I promise that a day in the park playing chess with your friends is better than any amount of time you can spend on the internet.
2. take to the streets. i think that protesting is effective, and more people should do it. if you're willing to take action, it models to others that they can take action.
3. stop coming back to the same machines. I think we all have this gamblers fallacy in us, where if it didn't pay out the first time, but it pays out sometimes, we try the machine again, but it doesn't really work like that. if you're losing money the lesson is to stop putting coins in the machine.




editor's note: I've been working on a free [[https://sprinklz.io][customizable news reader]] as a way to take out some of my frustrations with the modern internet. if you don't use this one, I recommend following individual blogs and websites, because it's so much better to just get off these big platforms entirely.
...
* Deft version 0.2.0
:PROPERTIES:
:CUSTOM_ID: deft-version-0-2-0
:RSS_PERMALINK: #deft-version-0-2-0
:PUBDATE:  2026-06-16
:ID:       65066912-9467-4D4A-BA4F-1F3835751ED8
:END:
Now that [[https://github.com/sstraust/deft][deft 0.2.0]] is released, I figure it's a good time to talk about what I built, and why I built it.
...
:PROPERTIES:
:CUSTOM_ID: how-much-llms-is-too-much-llms
:RSS_PERMALINK: #how-much-llms-is-too-much-llms
:PUBDATE:  2026-04-29
:ID:       4402BC1D-600D-4AE0-8B3A-87C146D3CB4E
:END:
...
Don't get me wrong, I love LLMs for the right purpose. I'd love to use LLMs on doctor's notes to build models that predict whether the flu is spreading. But when it comes to writing, especially this kind of writing, it just feels so unnecessary, because when you write down your thoughts, I already have exactly what I wanted, and it just feels so silly to add an extra stop in between.

As a poker player and game theorist, I'm sure Nate would appreciate the phrase "If you couldn't be bothered to write it, then I can't be bothered to read it". And you know, despite a belief that art can live separately from the artist, deep down for me this claim rings true. Not that the same words produced by a person are inherently different than the same words from an LLM, but it throws off my little Bayesian estimator a little.

If the smartest person I know spent 10,000 hours writing, editing, polishing, and publishing a 500 page novel, it tells me not just that the smartest person I know had some thoughts on a topic, but it tells me that they decided to spend 10,000 hours of /their/ precious time, and of /their/ precious energy to communicate this idea to me. It feels good that the time it took me to read it is porportional (and usually less than) the time it took you to write it, because it shows me that you care about the time and energy of creating this exchange.

And it's not just fully AI generated slop that hits this barrier. When you incorporate LLMs into your writing, even a little bit, you miss out on the ability to process your thoughts and create human connection.


* Building Chess in about 350 lines of Clojure
:PROPERTIES:
:CUSTOM_ID: building-chess-in-about-350-lines-of-clojure
:RSS_PERMALINK: #building-chess-in-about-350-lines-of-clojure
:PUBDATE:  2026-02-11
:ID:       C57E09F1-C89D-4EC5-BA95-729DD04FF03E
:END:
I recently released [[https://github.com/sstraust/deft][deft]], a Clojure library that defines types and protocols using plain maps, and so, to take it for a spin, I wrote the game of chess in Clojure.

Chess is a good trial run for any class and interface system, because it's reasonably small and self-contained (and has some pretty obvious choice for classes), but it also comes with some of the messiness of a real application, in that, you want to write something concise, but you still have to handle edge-cases, like castling and en passant.

I hand-drew all of the pieces, and used this little reagent snippet to render the display,
   ...
   (doall (for [x (range (count tiles))]
		  ...
		  [:v-box
		   (doall (for [y (reverse (range (count (nth tiles x))))]
			...
			"")]))]))]
#+end_src
but kind of the interesting bit is how to represent the board state and all of its pieces.

I did sort of the obvious thing, and defined types representing a chess piece, and then a board, as a 8x8 grid of pieces
#+begin_src clojurescript
...
and then for the piece behavior, I made protocols that define how a piece is displayed, and how it can move.
#+begin_src
(defp PieceType
  (image-path [this]))
...
here is an example for the knight:
#+begin_src clojurescript
(deft Knight []
  PieceType
...
is-valid-move-for-piece? returns a true-false value of "can my piece move to this square".


This works great for most pieces, but is insufficient for more complicated moves, like castling, where several pieces move at the same time. To handle this, we use another multimethod

#+begin_src
(defmulti special-move-for-piece (fn [x & args] (:type x)))
#+end_src

that is defined only for special moves, and returns the actual state of the modified gameboard.

so for pawns, it would look something like this:
#+begin_src clojurescript
...
I'm not going to copy-paste the movesets for every piece in this post, but it was pretty straightforward to define relevant methods for all the different piece types and special-cases, and I'm pretty happy with how the code turned out. When you're reading this code, it really looks like somebody just wrote down all the rules for the game of chess, and that's what makes me feel good, and feel like we've done things in a way that's well organized.


But now, sort of the most important question (for me) is:

is this library actually helping? did deft make our lives better?


I think it did, for the following reasons:
- deft gives us the features from both protocols/records and multimethods/plain-maps, so we didn't need to /decide/ which tool was right for our use-case. this removes just like a lot of cognitive load, because we can use one tool, and trust that it can do everything pretty well.
- in particular, it was nice to be able to define :default method dispatch for our protocols, and it was extremely nice to be able to reload protocol method defs from the repl without having to re-instantiate the entire chessboard (and clear the current position)
- it was really nice to be able to enforce when I wanted method defs to be defined for a particular type, and it was really nice to have an easy way to add Malli schema


Overall, I'm pretty happy with how it all turned out, and I'd say the test of our library was a success.

You can find the entire code for

the game here: https://github.com/sstraust/deft/blob/master/examples/src/examples/chess.cljs

the library here: https://github.com/sstraust/deft

and the detailed video explanation here: https://www.youtube.com/watch?v=lrofmFI1UJY
...
* Announcing Deft: A class and interface system for Clojure and ClojureScript
:PROPERTIES:
:CUSTOM_ID: announcing-deft-a-class-and-interface-system-for-clojure-and-clojurescript
:RSS_PERMALINK: #announcing-deft-a-class-and-interface-system-for-clojure-and-clojurescript
:PUBDATE:  2026-01-12
:ID:       F80E917B-3484-4CEC-84A7-E908A82EDCC9
:END:
Today I'm releasing deft! It's a library that replaces clojure's existing class system with plain maps and Malli schema.
...
Video Tutorial: https://www.youtube.com/watch?v=dlW6YzwUZ-M
On Hackernews: https://news.ycombinator.com/item?id=46590958
(plz star the repo!)

Synopsis:
This library came out of out frustrations with Clojure's existing type ecosystem. Both defrecord/defprotocol, and plain maps+multimethods, the way they are currently used, make Clojure programs fundamentally difficult to organize and refactor.
...
- Clojure maps are extremely difficult to nail down and make stable. Even with libraries like Malli, it's hard to declare interfaces that explain "what can I do with this thing?"
- Unless you are extremely regimented about defining Malli schemas for every single use case, it's easy for things to get disconnected during destructuring, because missed field lookups return nil
...
A secondary goal was to make something that is reliable and stable, and is (hopefully) easy to adopt in existing codebases. check it out!
* My Blog is Now on RSS
:PROPERTIES:
:CUSTOM_ID: my-blog-is-now-on-rss
:RSS_PERMALINK: #my-blog-is-now-on-rss
:PUBDATE:  2026-01-08
:ID:       027097A5-FF3F-4A2E-8A2B-287037706F14
:END:
if ur reading this i can publish to my blog w/rss.

im using org-rss-export-to-rss with some very simiple tooling. i thought about using hugo but idk it seemed too heavy and i kinda like the minimalist feel. im optimistic that hugo will be better when i set it up right, and i will probably need some of its features eventually, but thats a battle for another day.
* How to Win at Wordle
:PROPERTIES:
:CUSTOM_ID: how-to-win-at-wordle
:RSS_PERMALINK: #how-to-win-at-wordle
:PUBDATE:  2025-12-14
:ID:       59A3F135-6208-46E1-8B42-0E86486C895F
:END:
...
:PROPERTIES:
:ID:       81E51619-D781-40D7-9B49-2A5C61F7D4A0
:END:
...
:PROPERTIES:
:ID:       06422413-45DF-498B-9AEB-F8EBA92B5DD3
:END:
...
:PROPERTIES:
:ID:       6E6686E5-CA34-435E-9C68-FECDF5EA7227
:END:
...
:PROPERTIES:
:ID:       BC84053E-824F-4654-848D-B381DF4C69CB
:END:
...
:RSS_PERMALINK: #the-power-of-habit-book-review-pt-1
:PUBDATE:  2025-12-14
:ID:       CA27059E-BC57-4F5C-8729-5C598DCD352F
:END:
...
:RSS_PERMALINK: #modern-monetary-theory-and-you
:PUBDATE:  2025-12-14
:ID:       EC6B9B53-15FD-44FE-878E-A1142C9F9255
:END:
...
** The Big If
:PROPERTIES:
:ID:       8252C9B9-B35B-4580-8233-479C1705BA6D
:END:
...
** How We Got Here
:PROPERTIES:
:ID:       52818742-EBFF-448A-8E31-AD8F84E074E2
:END:
...
:PROPERTIES:
:ID:       73859BEF-D79B-470A-9A8F-04521A7A175B
:END:
...
:PROPERTIES:
:ID:       AADDCE98-FB55-42CE-B36D-A09F23E5DC16
:END:
...
:PROPERTIES:
:ID:       EC7A53E4-5B8D-4119-8CD3-EA779D1A083B
:END:
...
:RSS_PERMALINK: #the-perfect-camera
:PUBDATE:  2025-12-14
:ID:       04056E08-1438-4B99-AEB2-29934C2E34BD
:END:
...
:RSS_PERMALINK: #camera-lenses-what-s-in-my-bag
:PUBDATE:  2025-12-14
:ID:       B502DFBF-B509-4544-AF6F-5722B688AF06
:END:
...
:RSS_PERMALINK: #why-many-primary-forecasts-are-wrong
:PUBDATE:  2025-12-14
:ID:       2236584B-9A4E-44C3-841E-D2F478E61D3E
:END:
...
:RSS_PERMALINK: #the-wonky-math-behind-the-democratic-primary
:PUBDATE:  2025-12-14
:ID:       4D452D6B-1CCB-49E8-962D-F73D6955688F
:END:
...
:RSS_PERMALINK: #what-s-going-on-with-the-impeachment
:PUBDATE:  2025-12-14
:ID:       35CCC064-9539-4756-8A6F-A30D5806DD6B
:END:
...
:RSS_PERMALINK: #treating-cancer-with-chemo-part-3
:PUBDATE:  2025-12-14
:ID:       3ECEEE62-2DDA-4770-8ED0-5E17AC7372F2
:END:
...
:RSS_PERMALINK: #treating-cancer-with-chemo-part-2
:PUBDATE:  2025-12-14
:ID:       DA90CDF9-38CA-4742-8979-171B52E0F16B
:END:
...
:RSS_PERMALINK: #treating-cancer-with-chemo-part-1
:PUBDATE:  2025-12-14
:ID:       83434157-24D1-4EE8-92E1-9EBD49B7FF45
:END:
...
:RSS_PERMALINK: #the-importance-of-doing-nothing
:PUBDATE:  2025-12-14
:ID:       CF8B7FD0-A150-42DC-BB88-D3D2043E916D
:END:
...
:RSS_PERMALINK: #why-i-support-citizens-united
:PUBDATE:  2025-12-14
:ID:       003478E0-5EC3-45E5-8094-466AE12337DF
:END:
...
:RSS_PERMALINK: #camera-settings
:PUBDATE:  2025-12-14
:ID:       B2242E83-C459-48ED-8046-EEF72E69B051
:END:
